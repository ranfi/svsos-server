...
public class OrderDetailController {
	...
	public String list(HttpServletResponse response, HttpServletRequest request, Model model) {
		...
		model.addAttribute("sysurl", SystemGlobal.IMAGE_SERVER_URL);
	...
	@ResponseBody
	public Map<String, String> upload(@RequestParam MultipartFile fileToUpload, HttpServletRequest request)
			throws Exception {
		Map<String, String> result = Maps.newHashMap();
		try {

			String url = UUID.randomUUID().toString().replace("-", "");
			String imgPath = SystemGlobal.IMAGE_SAVE_PATH + url;
			String resourceSavePath = "" + imgPath + ".jpg";
			String imgPath1 = SystemGlobal.IMAGE_SERVER_URL + "/img/" + url;
			FileOutputStream out = new FileOutputStream(new File(resourceSavePath));
