...
public interface ServiceOrderDao extends PagingAndSortingRepository<ServiceOrder, Integer>{
	...
	@Query(value = "select * from ss_service_order t1 where t1.accept_worker_id=?1 and t1.create_time >= DATE_FORMAT(NOW(),'%Y-%m-%d') ORDER BY t1.create_time", nativeQuery = true)
	public List <ServiceOrder> findServiceOrderByacceptWorkerIdAndCurrentDate(Integer id);
	
	@Query(value = "select * from ss_service_order t1 where t1.accept_worker_id=?1 and t1.create_time >= DATE_FORMAT(DATE_SUB(now(),INTERVAL WEEKDAY(now()) day),'%Y-%m-%d') ORDER BY t1.create_time", nativeQuery = true)
	public List <ServiceOrder> findServiceOrderByacceptWorkerIdAndCurrentWeek(Integer id);
	
	@Query(value = "select * from ss_service_order t1 where t1.accept_worker_id=?1 and t1.create_time >= DATE_FORMAT(DATE_SUB( DATE_SUB( NOW( ) , INTERVAL DAYOFMONTH( NOW( ) ) -1 DAY ) , INTERVAL 0 MONTH ),'%Y-%m-%d') ORDER BY t1.create_time", nativeQuery = true)
