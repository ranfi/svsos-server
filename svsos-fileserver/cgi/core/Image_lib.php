...
class Image_lib {
    ...
    function initialize($props = array()) {
         ...
         */
        if (count($props) > 0) {
            $this->config = $props;   // by floren  new
    ...
    function image_process_gd($action = 'resize') {
        ...
        if ($action == 'crop') {
            $this->width = $this->config['width'];   // by floren new
            $this->height = $this->config['height'];

            //  Reassign the source width/height if cropping
            $this->orig_width = $this->config['orig_width'];
            $this->orig_height = $this->config['orig_height'];
