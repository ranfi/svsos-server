...
class Audio extends UploadFile implements FileProcess {

    private $paths;
    private $query;
    private $prefixFolder = "audio";
    private $audioPath;
    ...
    public function doUpload() {
        if (!empty($_FILES)) {
            ...
            foreach ($_FILES as $filename => $v) {
                ...
                $this->audioPath = FILE_FOLDER . $this->hashFolder;
    ...
    private function _upload($filename) {
        if (is_dir($this->audioPath)) {
            $userUploadFolderPath = $this->audioPath;
        } else {
            $userUploadFolderPath = FileHelper::createServerFolder($this->audioPath);
        }

        $config['upload_path'] = $userUploadFolderPath;
        $config['allowed_types'] = 'aac|amr|wav|mp3';
